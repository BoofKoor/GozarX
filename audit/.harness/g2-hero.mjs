...
const TREATMENTS = {
  ...
  "C · weight + size, colour demoted to a rule": `
    .hero-copy h1 .grad{
      background:linear-gradient(var(--hero-2),var(--hero-2)) no-repeat !important;
...
const run = async () => {
  ...
  for (const [name, css] of Object.entries(TREATMENTS)) {
    ...
    for (const theme of ["dark", "light"]) {
      ...
      await page.screenshot({
        path: `${OUT}/hero-${name.split(" ")[0]}-${theme}.png`,
  ...
  await browser.close();
  await writeFile(`${OUT}/g2-hero.json`, JSON.stringify(results, null, 2));
